                ...
                <div class="max-w-3xl mx-auto space-y-8 text-white">
                    ...
                    @if(count($dawuhs) > 0)
                        <div x-data="{ 
                            ...
                            x-init="if (dawuhs.length > 1) { setInterval(() => { activeDawuh = (activeDawuh + 1) % dawuhs.length }, 10000) }">
                            <div
                            ...
                            <template x-for="(d, index) in dawuhs" :key="index">
                                <div x-show="activeDawuh === index" x-transition:enter="transition ease-out duration-700"
                                    x-transition:enter-start="opacity-0 translate-y-4"
                                    x-transition:enter-end="opacity-100 translate-y-0"
                                    x-transition:leave="transition ease-in duration-500 absolute inset-x-0"
                                    x-transition:leave-start="opacity-100 translate-y-0"
                                    x-transition:leave-end="opacity-0 -translate-y-4"
                                    class="bg-background-dark/40 backdrop-blur-xl p-8 rounded-2xl border-l-4 border-accent max-w-2xl mx-auto mt-6 shadow-2xl relative">
                                    ...
                                    <p class="text-sm font-extrabold text-accent mt-4 uppercase tracking-[0.2em]"
                                        x-text="'— ' + d.ulama_title + ' ' + d.ulama_name"></p>
                                </div>
                            </template>
                        </div>
                    @else
